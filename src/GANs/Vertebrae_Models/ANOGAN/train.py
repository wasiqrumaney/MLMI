...
def ANO(args):
    ...
    print('#'*80)
    print("Will try my best to do Anomaly detection, Sir")
    print('#'*80)
    hyparam_list = [("model", args.model_name),
                    ("cube", args.cube_len),
                    ("bs", args.batch_size),
    ...
    N=10 # None for the whole dataset
    VOL_SIZE = 64
    train_path = pathlib.Path("../../Vert_dataset")
    dataset = VertDataset(train_path,n=N , transform=transforms.Compose([ResizeTo(VOL_SIZE),
                                                                   transforms.ToTensor()]))
    ...
    #Load checkpoint if available
    read_pickle(pickle_path, G, G_solver, D, D_solver)
    X = dataset[9] # loading the first real sample
    #
    #X = X.unsqueeze(0)
    #print(X.shape)
    #X = X.view(-1,args.cube_len*args.cube_len*args.cube_len)
    X = var_or_cuda(X)
    X = X.type(torch.cuda.FloatTensor)
    Z = Variable(generateZ(num_samples = 1, z_size = args.z_size), requires_grad = True).cuda()
    #save the first generation 
    if not os.path.exists('ano_out'):
       os.makedirs('ano_out/gen')
       os.makedirs('ano_out/orig')
       os.makedirs('ano_out/diff')

    first_gen = G(Z).cpu().data.squeeze().numpy().reshape(-1,args.cube_len,args.cube_len,args.cube_len)
    Save_Voxels(first_gen, 'ano_out/gen', "1st_gen")

    print("Z shape", Z.shape)
    for i in range(args.train_ano):
        G_z = G(Z) # generate image given a random noise vector z
        G_z = G_z.view(args.cube_len,args.cube_len,args.cube_len)
        #print("G_z shape ",G_z.shape)

        # create the latent space optimizer
        z_optimizer = torch.optim.Adam([Z],lr=1e-1, betas=(0.9,0.99))

        # Compute residual loss
        residual_loss = torch.sum(torch.abs(X-G_z))

        # compute discrimination loss 
        _, feat_X = D(X.unsqueeze(0))
        _, feat_G_z = D(G_z.unsqueeze(0))

        #print(feat_X.shape, feat_G_z.shape)
        discrimination_loss = torch.sum(torch.abs(feat_X-feat_G_z))

        # Propagate the total loss 
        ano_param=0.1
        total_loss = (1-ano_param)*residual_loss + ano_param*discrimination_loss
        total_loss.backward()
        z_optimizer.step()

        if i%50 == 0: 
           print("Iter-{};total_loss {:0.4} ; res_loss {:0.4}; discr_loss {:0.4}; ".format(i, total_loss.item(), residual_loss.item(), discrimination_loss.item()))
    
    # generation using the final Z vector 
    final_retrieval = G(Z)
    samples = final_retrieval.cpu().data.squeeze().numpy()
    samples = samples.reshape(-1,args.cube_len,args.cube_len,args.cube_len)

    #saving the generation
    Save_Voxels(samples, 'ano_out/gen', "gen")
    #saving original query
    orig = X.cpu().data.unsqueeze(0).numpy()
    #print(X.shape, orig.shape)
    Save_Voxels(orig, 'ano_out/orig', "orig")
    #save the diff voxels
    diff = (X-G_z).cpu().data.unsqueeze(0).numpy()
    Save_Voxels(diff, 'ano_out/diff', "diff")        
