...
#                 padd = (1,1,1)
    ...
    def forward(self, x):
        ...
        out = self.layer1(out)     
        #print("G:L1",out.size())  #torch.Size([64, 256, 4, 4, 4])
        out = self.layer2(out)
        #print("G:L2",out.size()) # torch.Size([64, 128, 8, 8, 8])
        out = self.layer3(out)
        #print("G:L3",out.size()) # torch.Size([64, 64, 16, 16, 16])
        out = self.layer4(out)
        #print("G:L4",out.size()) #  torch.Size([64, 1, 32, 32, 32])
...
class _D(torch.nn.Module):
    def __init__(self, args):
        ...
        self.layer5 = torch.nn.Sequential(
            torch.nn.Conv3d(self.cube_len*16, 1, kernel_size=4, stride=2, bias=args.bias, padding=(0,0,0)),
            #torch.nn.LeakyReLU(self.args.leak_value)
            #torch.nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        ...
        #print("start",out.size()) #  torch.Size([64, 1, 32, 32, 32])
        out = self.layer2(out)
