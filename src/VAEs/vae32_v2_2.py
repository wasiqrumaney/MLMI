...
class Vae32(nn.Module):
    def __init__(self, z_dims, debug=False, device="cpu"):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(self.z_dims, 96),
            Reshape(DEC_INSHAPE),
            nn.ConvTranspose3d(96, 64, kernel_size=3, stride=2, padding=0),  #
            nn.ReLU(True),
            nn.ConvTranspose3d(64, 64, kernel_size=4, stride=1, padding=0),  #
            nn.ReLU(True),
            nn.ConvTranspose3d(64, 32, kernel_size=3, stride=2, padding=0),  #
            nn.ReLU(True),
            nn.ConvTranspose3d(32, 16, kernel_size=4, stride=1, padding=0),  #
            nn.ReLU(True),
    ...
    def encode(self, x):
        ...
        for module in self.encoder:
            y = module(y)
            if self.debug and not isinstance(module, nn.ReLU): print(y.size())
        mu = self.fc_mu(y)
    ...
    def decode(self, z):
        ...
        for module in self.decoder:
            rec = module(rec)
            if self.debug and not isinstance(module, nn.ReLU): print(rec.size())
        return rec
